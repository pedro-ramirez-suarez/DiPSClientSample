...
function messageReceived(from, message, apply) {
    ...
    var hash = MD5(from);
    //if the message is from me, do not add it
    if (myId == from)
        from = 'me';

    scope.messages.push({ message: message, from: from, date: date,  hash : hash });
    if (apply)
        scope.$apply();
}

function sendMessage(messageTo, message) {
    var msg = { From: myId, MessageText: message };
    diPSClient.Publish('newmessage', msg);
}
...
function updateUserList(data) {
    ...
    for (x in data) {
        var usr = data[x];
        var hash = MD5(usr.ClientId);
        scope.users.push({ ClientId: usr.ClientId, Hash: hash });
    }
    scope.$apply();
}
